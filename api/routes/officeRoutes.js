...
router.get('/properties', isLoggedIn, async (req, res) => {
	...
	res.render('./office/properties.ejs', {response:jsonData, location:locationData, sold: false});
});
...
router.get('/profile/edit',isLoggedIn,async (req,res) =>{
		...
		res.render('./office/editagentprofile.ejs',{response:jsonData,response1:jsonMobile});
});
...
router.post('/editagentprofile/:id',isLoggedIn,async (req,res) =>{
	...
	await local({
		...
		if(response.status == 201) {
            res.render('/office/'+req.session.userid);
        }
	}).catch(err => {
        res.redirect('/pageNotFound')
    })
...
router.post('/deleteclient', isLoggedIn, async (req, res) => {
	...
	await local({
		...
		if(response.status == 201) {
			...
			res.redirect('/officeUser/clients');
        }
	}).catch(err => {
...
router.post('/deleteagent', isLoggedIn, async (req, res) => {
	...
	await local({
		...
		if(response.status == 201) {
			...
			res.redirect('/officeUser/agents');
        }
	}).catch(err => {
