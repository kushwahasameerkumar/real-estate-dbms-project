const propertyCard = (name, address, bedrooms, bathrooms, balconies, size, category, img, priceString) => {
...
function searchProperty(){
    ...
    fetch('http://localhost:3000/api/property/search?'+url.params).then(res => res.json()).then(res => {
        ...
        properties.forEach(property => {
            const prop = propertyCard(property.name, property.address, property.bedrooms, property.bathrooms, property.balconies, property.size, property.category, property.img, property.priceString);
            appendProperty(prop);
        });
    });
}
