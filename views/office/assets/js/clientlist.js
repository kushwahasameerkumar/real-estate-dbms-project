const tableRow = (id,name,address,email,aadhaar, dob) => {
    return `<tr>
    <td><a href="/officeUser/client/${id}">${id}</a></td>
    <td>${name}</td>
    <td>${address}</td>
    <td>${email}</td>
    <td>${dob}</td>
    <td><form action="/officeUser/deleteclient" method="POST"><input id="id" name= "id" style="display: none;" value="${id}"><button type="submit" id="delete">x</button></form></td>
</tr>`;
};
